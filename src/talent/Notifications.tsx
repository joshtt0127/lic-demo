...
function destination(type: string, entityType: string | null): string {
  if (type === 'message' || entityType === 'conversation') return '/talent/messages'
  if (type === 'application_status' || entityType === 'application') return '/talent/auditions'
  return '/talent/casting-calls'
}
        ...
        <div className="flex flex-col gap-2">
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.id}
                to={destination(item.type, item.entity_type)}
